...
namespace abclib::ui
{
    ...
    /**
     * @brief Format a vector of numeric values without validity checking
     *
     * Always shows values, never shows "---". Use for data that should
     * always be displayed regardless of validation status.
     *
     * @param values Vector of values to format
     * @param format Printf-style format string for each value (e.g. "%.0f" for integers)
     * @return Formatted string like "[1205, 823, 450, 920]"
     */
    template <typename T>
    inline std::string format_value_vector_always(
        const std::vector<T> &values,
        const char *format)
    {
        std::string result = "[";
    ...
    /**
     * @brief Format a vector of walls WITHOUT validity checking
     *
     * @param walls Vector of wall enums
     * @return Formatted string like "[N, S, E, W]"
     */
    inline std::string format_wall_vector_always(
        const std::vector<field::FieldMap::Wall> &walls)
    {
        std::string result = "[";
        for (size_t i = 0; i < walls.size(); i++)
