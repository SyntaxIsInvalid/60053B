...
namespace abclib::trajectory
{
        ...
        const FollowerConfig &config)
    {
        ...
        else if (const path::ProfileGroup *group = trajectory.get_profile_group())
        {
            // Multi-segment mode: check if this profile group is a turn-in-place
            is_turn_in_place = group->is_turn_in_place_group();
        }
