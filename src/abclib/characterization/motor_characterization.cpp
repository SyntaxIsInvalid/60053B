...
namespace abclib::characterization
{
        ...
        int test_duration_ms)
    {
        ...
        // Build filename with voltage and rotation direction suffix
        char full_filename[64];
        snprintf(full_filename, sizeof(full_filename), "/usd/%s_%.1fV_%s.csv",
                 filename, constant_voltage, ccw_rotation ? "ccw" : "cw");

        pros::lcd::print(0, "Starting kA turn test...");
        pros::lcd::print(1, "Voltage: %.1fV", constant_voltage);
        pros::lcd::print(2, "Rotation: %s", ccw_rotation ? "CCW" : "CW");
        pros::delay(2000);
        ...
        // Apply constant voltage with opposite signs for turning
        units::Voltage left_voltage = units::Voltage::from_volts(constant_voltage * left_multiplier);
        units::Voltage right_voltage = units::Voltage::from_volts(constant_voltage * right_multiplier);
        ...
        while ((pros::millis() - test_start_time) < static_cast<uint32_t>(test_duration_ms))
        {
            ...
            units::MotorAngularVelocity left_vel = left.get_raw_velocity();
            units::MotorAngularVelocity right_vel = right.get_raw_velocity();
            double avg_vel_rad_s = (std::abs(left_vel.rad_per_sec) + std::abs(right_vel.rad_per_sec)) / 2.0;

            uint32_t timestamp = pros::millis() - test_start_time;

            // Write to SD card
            fprintf(file, "%lu,%.2f,%.4f,%.4f,%.4f\n",
                    timestamp, constant_voltage,
                    left_vel.rad_per_sec, right_vel.rad_per_sec, avg_vel_rad_s);
        ...
        pros::lcd::print(0, "kA turn test complete!");
        pros::lcd::print(1, "File: %s", full_filename);
        pros::lcd::print(2, "Analyze for kA_turn value");
    }
}