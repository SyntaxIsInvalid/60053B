...
namespace abclib::estimation
{
    ...
    void EKFOdometryEstimator::initialize_ekf()
    {
        ...
        // Measurement noise (empty for prediction-only)
        Eigen::Matrix<double, 0, 0> measurement_noise;

        ekf_ = filters::ExtendedKalmanFilter<3, 0>(
            initial_state,
            initial_covariance,
            process_noise,
            measurement_noise
    ...
    void EKFOdometryEstimator::reset()
    {
        ...
        if (vertical_model_) vertical_model_->reset();
        if (horizontal_model_) horizontal_model_->reset();
        if (imu_model_) imu_model_->reset();
    ...
    void EKFOdometryEstimator::update()
    {
        ...
        units::Length delta_vertical = vertical_model_ ? 
            vertical_model_->get_measurement() : units::Length::from_inches(0.0);
        units::Length delta_horizontal = horizontal_model_ ? 
            horizontal_model_->get_measurement() : units::Length::from_inches(0.0);
        units::Angle delta_imu = imu_model_ ? 
            imu_model_->get_measurement() : units::Angle::from_radians(0.0);
        ...
        // EKF prediction step (no correction for now)
        ...
        if (!first_update_)
        {
            ...
            double v_raw = (dx * std::cos(current_pose_.theta_rad()) +
                            dy * std::sin(current_pose_.theta_rad())) / dt;
    ...
    void EKFOdometryEstimator::set_pose(const Pose& pose)
    {
        ...
        state << pose.x.value(),  // Already in meters (SI units)
                 pose.y.value(),
                 pose.theta.value();
    ...
    void EKFOdometryEstimator::set_process_noise(const Eigen::Matrix3d& process_noise)
    {
        ekf_.set_process_noise(process_noise);
    }
