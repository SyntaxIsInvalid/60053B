...
namespace abclib::path
{
        ...
        const Point &pos_b, const Point &vel_b, const Point &accel_b)
    {
        ...
        x_coeffs_(3) = -10.0 * pos_a(0) - 6.0 * vel_a(0) - 1.5 * accel_a(0) + 10.0 * pos_b(0) - 4.0 * vel_b(0) + 0.5 * accel_b(0);
        x_coeffs_(4) = 15.0 * pos_a(0) + 8.0 * vel_a(0) + 1.5 * accel_a(0) - 15.0 * pos_b(0) + 7.0 * vel_b(0) - 1.0 * accel_b(0);
        x_coeffs_(5) = -6.0 * pos_a(0) - 3.0 * vel_a(0) - 0.5 * accel_a(0) + 6.0 * pos_b(0) - 3.0 * vel_b(0) + 0.5 * accel_b(0);

        // Y-dimension coefficients
        y_coeffs_(0) = pos_a(1);
        y_coeffs_(1) = vel_a(1);
        y_coeffs_(2) = 0.5 * accel_a(1);
        y_coeffs_(3) = -10.0 * pos_a(1) - 6.0 * vel_a(1) - 1.5 * accel_a(1) + 10.0 * pos_b(1) - 4.0 * vel_b(1) + 0.5 * accel_b(1);
        y_coeffs_(4) = 15.0 * pos_a(1) + 8.0 * vel_a(1) + 1.5 * accel_a(1) - 15.0 * pos_b(1) + 7.0 * vel_b(1) - 1.0 * accel_b(1);
        y_coeffs_(5) = -6.0 * pos_a(1) - 3.0 * vel_a(1) - 0.5 * accel_a(1) + 6.0 * pos_b(1) - 3.0 * vel_b(1) + 0.5 * accel_b(1);
    }
    ...
    {
        const Point d1 = calc_first_deriv(u);
        const Point d2 = calc_second_deriv(u);

        // Signed curvature: κ = (x'y'' - y'x'') / (x'² + y'²)^(3/2)
        const double numerator = d1.x() * d2.y() - d1.y() * d2.x(); // no std::abs()
    ...
    void QuinticHermiteSegment::build_arc_length_table()
    {
        arc_table_.build([this](double u)
                         { return calc_first_deriv(u).norm(); });
    }

    double QuinticHermiteSegment::arc_length_to_u(double s) const
    {
        return arc_table_.arc_length_to_u(s);
    }

    double QuinticHermiteSegment::u_to_arc_length(double u) const
    {
